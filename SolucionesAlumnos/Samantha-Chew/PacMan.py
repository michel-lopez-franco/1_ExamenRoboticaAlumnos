...
Versión básica de Pac-Man con autopilot que recoge todos los pellets.
Los fantasmas aparecen dibujados pero permanecen quietos e ignorados.
...
Entrada:
    maze (list[str]) → lista de strings con el laberinto.
    Símbolos:
        '#' : pared
        '.' : pellet
        'o' : power pellet
        'P' : inicio Pac-Man
        'G' : fantasma (dibujado pero ignorado en autopilot)
        ' ' : espacio vacío

Salida:
    dict con:
        "positions": lista de posiciones recorridas (fila,col)
        "moves": lista de movimientos en texto
        "steps": número total de pasos
        "time": tiempo de ejecución (s)

==================== Personalización añadida ====================

- Opción para elegir diferentes mapas con el parámetro --map (1, 2 o 3).
...
El agente utiliza búsqueda A* (A estrella) para planificar rutas entre
Pac-Man y el pellet más cercano en Manhattan. Esta estrategia greedy + A*
es eficiente en laberintos moderados y garantiza rutas válidas que no atraviesan muros.
"""

import arcade, heapq, time, argparse
...
# ===================== CONFIG =====================
SCREEN_TITLE = "PacGPT5 Autopilot"
TILE_SIZE = 32
MOVEMENT_SPEED = 4
...
class PacGPT5(arcade.Window):
    ...
    def setup(self):
        ...
        colors = [arcade.color.RED, arcade.color.GREEN, arcade.color.PINK, arcade.color.PURPLE]
    ...
    def on_draw(self):
        ...
        if self.state == "WIN":
            arcade.draw_text("¡GANASTE!", SCREEN_WIDTH/2, SCREEN_HEIGHT/2,
    ...
    def on_update(self, delta_time: float):
        ...
        for pw in arcade.check_for_collision_with_list(self.pacman, self.power_list):
            pw.remove_from_sprite_lists(); self.pacman.score += 50; self.pacman.power_timer = 7.0
        if not self.pellet_list and not self.power_list:
            self.state = "WIN"
            print("¡GANASTE!")
...
def solve(maze: List[str]) -> dict:
    ...
    end_time=time.time()
    return {"positions":positions,"moves":moves,"steps":len(moves),"time":round(end_time-start_time,4)}

# ===================== MAIN =====================
def main():
    global RAW_MAP, ROWS, COLS, SCREEN_WIDTH, SCREEN_HEIGHT
    parser = argparse.ArgumentParser(description="PacGPT5 Autopilot con mapas")
    ...
    ROWS, COLS = len(RAW_MAP), len(RAW_MAP[0])
    SCREEN_WIDTH, SCREEN_HEIGHT = COLS * TILE_SIZE, ROWS * TILE_SIZE
    game = PacGPT5(); game.setup(); arcade.run()
